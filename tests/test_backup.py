...
class TestKeepChanges:
    ...
    def test_copies_missing_even_when_last_run_in_future(
        ...
        """Нет особого «первого авто»: файл без архива копируется, даже если last_run в будущем."""
    ...
    def test_automatic_run_skips_old_files_already_in_archive(
        ...
        """Повторный автозапуск: старый по mtime файл, уже в архиве, не копируется снова."""
    ...
    def test_automatic_copies_missing_file_with_old_mtime(
        ...
        всё равно попадает в архив (его ещё нет в архиве).
        """
...
class TestLayered:
    ...
    def test_layered_copies_missing_with_old_mtime(
        ...
        """Новый файл со старым mtime попадает в следующий слой (его не было в архиве)."""
        src = test_root / "src6b"
